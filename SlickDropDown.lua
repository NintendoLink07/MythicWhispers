...
function SlickDropDown:SetRightClickMenuMode()
	self.Selected:Hide()
	self.List:ClearAllPoints()
	self.List:SetPoint("TOPLEFT", self, "TOPLEFT")
	self.List:AdjustPointsOffset(0, 0)
end

function SlickDropDown:Click()
	PlaySound(SOUNDKIT.IG_MAINMENU_OPTION_CHECKBOX_ON)

	local visible = self.List:IsShown()
	self.List:SetShown(not visible)

	if(self.currentList) then
		self.currentList:Hide()
	end
...
function SlickDropDown:CreateEntryFrame(info)
	if(not InCombatLockdown()) then
		...
		if(infoTable.disabled or infoTable.radioHidden) then
			frame.Radio:Hide()
